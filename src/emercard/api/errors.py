...
def card_exception_handler(request: Request, exc: CardError) -> JSONResponse:
    mapping: dict[type[CardError], tuple[int, str, str]] = {
        ...
        CardDirectOwnershipError: (
            409,
            "card.direct_ownership_retired",
            "Thao tác gán chủ thẻ trực tiếp đã được thay thế bằng liên kết hồ sơ.",
        ),
        ...
        CardLinkNotBoundError: (
            409,
            "card.link_not_bound",
            "Thẻ chưa được gắn với liên kết hồ sơ.",
        ),
        CardProfileLinkInvalidError: (
            409,
            "card.profile_link_invalid",
            "Liên kết hồ sơ không hợp lệ.",
        ),
        CardLinkRebindTargetInvalidError: (
            409,
            "card.link_rebind_target_invalid",
            "Chỉ có thể gắn liên kết hồ sơ đang chờ xử lý.",
        ),
        CardLinkTerminalError: (
            409,
            "card.link_terminal",
            "Liên kết thẻ đã ở trạng thái kết thúc.",
        ),
        CardPostDeliveryRebindError: (
            409,
            "card.post_delivery_rebind",
            "Không thể thay đổi liên kết của thẻ đã bàn giao.",
        ),
        CardEncodingNotVerifiedError: (
            409,
